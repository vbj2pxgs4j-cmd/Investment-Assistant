...
@pytest.fixture
def client():
    """Provide TestClient against the FastAPI application."""
...
def test_rate_limit_telemetry_endpoint(client: TestClient):
...
def test_chat_query_factual(client: TestClient):
...
def test_chat_query_pii_interception(client: TestClient):
...
def test_chat_query_advisory_refusal(client: TestClient):
...
def test_chat_query_comparison_refusal(client: TestClient):
...
def test_chat_query_ambiguous_scheme_disambiguation(client: TestClient):
...
def test_chat_query_validation_error(client: TestClient):
    ...
    # Missing query field
    response_missing = client.post("/api/v1/chat/query", json={})
